...
mcLogistic<-function(years)
{
  sqlite    <- dbDriver("SQLite")
  ncaaDB1 <- dbConnect(sqlite,"data/ncaa.db")
  dbListTables(ncaaDB1)
  
  # use all of the data available (we should have 2010-11, 11-12, 12-13, 13-14 data)??
  if (length(years) > 1)
  {
    game_data <- get_train_data(ncaaDB1, years[1])
    for (i in 2:length(years))
    {
      game_data <- rbind(game_data, get_train_data(ncaaDB1, years[i]))
    }
  }
  else
  {
    game_data <- get_train_data(ncaaDB1, years)
  }
  ...
  for (i in 1:len)
  {
    ...
    if (noMatches <=2)
    {
      ...
      { 
        ...
        next # just ignore this one and continue
      }
  ...
  fit <- glm(cbind(win, num-win) ~ pointDiff, data = empiricalRxHGroup, family = "binomial")
  fitCoef <- coef(fit)
